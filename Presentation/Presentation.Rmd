...
:::: column
\begin{itemize}
\item \textbf{Model}: $Weight = \beta_0 + \times \text{1}_{\{Man\}} + \epsilon$
\item $\beta_0 = -63$ and $\beta_1 = 0.81$, we also test the non-nullity of each coefficient 
\item $R^{2}$, the proportion of variance explained by our model, is 46%
\end{itemize}
::::
:::: column
```{r echo=FALSE, fig.align='center'}
...
:::: column
\begin{itemize}
\item \textbf{Modèle} : $Poids = \beta_0 + \beta_1 \times Age +  \beta_2 \times Taille + \epsilon$
